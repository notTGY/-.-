...
Для того чтобы сделать серверный рендеринг остаётся лишь написать полифилл на компонент для nodejs. Это реализовано [вот тут](https://github.com/notTGY/ussr-app-template/blob/master/happyFramework/ssr.js).
В [репке](https://github.com/notTGY/ussr-app-template/) настроено приложение с серверным рендерингом. Можно удостовериться в консоли, что рендеринг происходит на сервере.
В качестве примера создаётся небольшой `express` сервер в файле [server.js](https://github.com/notTGY/ussr-app-template/blob/master/server.js). Там СССР используется в строках:
...
Тут `html` - заготовка html'ки, `App` - счастливое приложение, а третим параметром передаются дополнительные параметры бекенда (сюда можно отправлять `path` или иные данные).

СССР может быть полезен везде (и это понадобилось мне), так что я решил сделать компонент `ssr.js`. На этот файл не распространяются обычные ограничения в размер бандла до `1024 байт`, так что сюда я скорее всего буду возвращаться для улучшений и дополнений.

Из этого можно понять, что для совершенста необходимо только одно.
